...
type FormMedicamento = Omit<Medicamento, "id" | "estado" | "categoria"> & {
  id?: number;
};
...
interface ModalMedicamentoProps {
  ...
  esAdmin: boolean;
  onGuardar: (form: FormMedicamento, stockCambio: boolean) => Promise<void>;
  onCerrar: () => void;
}
...
function ModalMedicamento({
  ...
  // El admin SÍ puede editar el stock actual de un medicamento existente.
  // El farmacéutico solo lo define al crear uno nuevo.
  const puedeEditarStock = esNuevo || esAdmin;
  ...
  const handleChange = <K extends keyof FormMedicamento>(
    campo: K,
    valor: FormMedicamento[K],
  ) => {
  ...
  const handleSubmit = async () => {
    ...
    if (!NOMBRE_REGEX.test(nombre)) {
      setError(
        "El nombre del medicamento debe tener entre 2 y 100 caracteres, sin símbolos no permitidos.",
      );
      return;
    }
    ...
    if (ubicacion.length < 2 || !TEXTO_LIBRE_REGEX.test(ubicacion)) {
      setError(
        "La ubicación debe tener al menos 2 caracteres y no contener símbolos no permitidos.",
      );
      return;
    }
    ...
    if (puedeEditarStock) {
      ...
      if (
        !Number.isFinite(stockActualNum) ||
        !Number.isInteger(stockActualNum) ||
        stockActualNum < 0
      ) {
        setError(
    ...
    if (
      !Number.isFinite(stockMinimoNum) ||
    ...
    try {
      const stockCambio =
        !esNuevo &&
        esAdmin &&
        formFinal.stockActual !== medicamento?.stockActual;
      await onGuardar(formFinal, stockCambio);
    } catch (err: any) {
      console.error(err);
      setError(
        err?.response?.data?.error ||
          "Ocurrió un error al guardar el medicamento. Intenta de nuevo.",
      );
    } finally {
      setGuardando(false);
    }
        ...
        <div className="p-4 d-flex flex-column gap-3">
          ...
          <div className="row g-2">
            <div className="col-md-4">
              ...
              <input
                ...
                title={
                  !puedeEditarStock
                    ? "Solo un administrador puede modificar el stock actual"
                    : undefined
                }
              />
            </div>
          ...
          {!esNuevo && !esAdmin && (
            <p className="fs-11 text-secondary mb-0">
              El stock actual solo lo puede modificar un administrador, o se
              actualiza automáticamente al entregar recetas.
            </p>
          )}
...
export default function GestionInventario() {
  const { medicamentos, categoriasMedicamento, cargando, usuarioActual } =
    useClinicaStore();
  const esAdmin = usuarioActual?.rol === "Administrador";

  const [busqueda, setBusqueda] = useState<string>("");
  const [modalMedicamento, setModalMedicamento] = useState<
    Medicamento | null | undefined
  >(undefined);

  // Refresca el inventario al entrar a la pantalla, para ver stock actualizado
  // por el admin (u otro farmacéutico) desde otra sesión.
  useEffect(() => {
    clinicaStore.refrescarMedicamentos();
  }, []);
  ...
  const estadoStock = (m: Medicamento) => {
    if (m.stockActual === 0)
      return { label: "Agotado", clase: "badge-soft-red" };
    if (m.stockActual <= m.stockMinimo)
      return { label: "Stock bajo", clase: "badge-soft-amber" };
  ...
  const guardarMedicamento = async (
    ...
    if (form.id) {
      // El stock actual se guarda por una vía separada (solo admin);
      // el resto de campos van por la actualización normal.
      if (stockCambio) {
        await clinicaStore.actualizarStockMedicamento(
          form.id,
          form.stockActual,
        );
      }
          ...
          {cargando ? (
            ...
            <>
              <div className="row row-cols-3 g-3 mb-4">
                <div className="col">
                  <StatCard label="Medicamentos" value={stats.total} />
                </div>
                <div className="col">
                  <StatCard
                    label="Stock bajo"
              ...
              <div className="border rounded overflow-hidden">
                <div
                  ...
                  style={{
                    display: "grid",
                    gridTemplateColumns: COLUMNAS_TABLA_INVENTARIO,
                    letterSpacing: ".03em",
                  }}
                >
                ...
                {medicamentosFiltrados.length === 0 && (
                  <p className="px-3 py-5 text-center fs-6 text-secondary mb-0">
                ...
                {medicamentosFiltrados.map((m) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        <span className={`badge-soft ${estado.clase}`}>
                          {estado.label}
                        </span>
                      </div>
                      <div className="d-flex align-items-center justify-content-center gap-1">
                        ...
                        <button
                          onClick={() =>
                            clinicaStore.toggleEstadoMedicamento(m.id)
                          }
                          className="btn btn-outline-secondary btn-icon-sm bg-white text-secondary"
                          title={activo ? "Desactivar" : "Activar"}
                        >
                          <i
                            className={`bi ${activo ? "bi-lock-fill" : "bi-unlock-fill"}`}
                            aria-hidden="true"
                          />
                        </button>
                      </div>
