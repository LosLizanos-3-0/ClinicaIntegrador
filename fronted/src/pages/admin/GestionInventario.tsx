...
function ModalMedicamento({ medicamento, categorias, esAdmin, onGuardar, onCerrar }: ModalMedicamentoProps) {
        ...
        <div className="p-4 d-flex flex-column gap-3">
          ...
          {!esNuevo && !esAdmin && (
            <p className="fs-11 text-secondary mb-0">
              El stock actual solo lo puede modificar un administrador, o se actualiza automáticamente al entregar recetas.
            </p>
          )}
...
export default function GestionInventario() {
  const { medicamentos, categoriasMedicamento, cargando, usuarioActual } = useClinicaStore();
  ...
  const guardarMedicamento = async (form: FormMedicamento, stockCambio: boolean) => {
    if (form.id) {
      // El stock actual se guarda por una vía separada (solo admin);
      // el resto de campos van por la actualización normal.
      if (stockCambio) {
        await clinicaStore.actualizarStockMedicamento(form.id, form.stockActual);
      }
