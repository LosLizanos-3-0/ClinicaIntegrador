...
type FormCategoria = Omit<CategoriaMedicamento, "id" | "estado"> & {
  id?: number;
};
...
function ModalCategoria({
  ...
  const handleChange = <K extends keyof FormCategoria>(
    campo: K,
    valor: FormCategoria[K],
  ) => {
...
export default function GestionCategoriaMedicamento() {
  ...
  const [modalCategoria, setModalCategoria] = useState<
    CategoriaMedicamento | null | undefined
  >(undefined);
  ...
  const guardarCategoria = async (form: FormCategoria) => {
    if (form.id) {
      await clinicaStore.actualizarCategoriaMedicamento({
        ...form,
        id: form.id,
      });
    } else {
          ...
          {cargando ? (
            ...
            <>
              <div className="row row-cols-2 g-3 mb-4">
                <div className="col">
                  <StatCard label="Categorías" value={stats.total} />
                </div>
                <div className="col">
                  <StatCard
                    label="Activas"
              ...
              <div className="border rounded overflow-hidden">
                <div
                  ...
                  style={{
                    display: "grid",
                    gridTemplateColumns: COLUMNAS_TABLA_CATEGORIAS,
                    letterSpacing: ".03em",
                  }}
                >
                  <span>Categoría</span>
                ...
                {categoriasFiltradas.map((c) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div>
                        <span
                          className={`badge-soft ${activa ? "badge-soft-green" : "badge-soft-gray"}`}
                        >
                      ...
                      <div className="d-flex align-items-center justify-content-center gap-1">
                        ...
                        <button
                          onClick={() =>
                            clinicaStore.toggleEstadoCategoriaMedicamento(c.id)
                          }
                          className="btn btn-outline-secondary btn-icon-sm bg-white text-secondary"
                          title={activa ? "Desactivar" : "Activar"}
                        >
                          <i
                            className={`bi ${activa ? "bi-lock-fill" : "bi-unlock-fill"}`}
                            aria-hidden="true"
                          />
                        </button>
                      </div>
