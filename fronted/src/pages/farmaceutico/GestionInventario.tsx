...
function ModalMedicamento({
  ...
  // El farmacéutico solo define el stock actual al crear un medicamento nuevo.
  // Al modificar uno existente, el stock queda siempre bloqueado (se actualiza
  // automáticamente al entregar recetas).
  const puedeEditarStock = esNuevo;
  ...
  const handleSubmit = async () => {
    ...
    // ── Stock actual (obligatorio solo al crear, el único momento editable) ─
    ...
    try {
      ...
      setError(
        err?.response?.data?.error ||
          "Ocurrió un error al guardar al guardar medicamento. Intenta de nuevo.",
      );
    } finally {
      setGuardando(false);
    }
          ...
          <div className="row g-2">
            <div className="col-md-4">
              ...
              <input
                ...
                title={
                  !puedeEditarStock
                    ? "El stock actual no se puede modificar desde este formulario"
                    : undefined
                }
              />
            </div>
