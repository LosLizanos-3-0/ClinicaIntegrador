...
interface ModalPacienteProps {
  paciente?: Paciente;
  onGuardar: (form: FormPaciente) => Promise<string | void>;
  onCerrar: () => void;
}
...
function ModalPaciente({ paciente, onGuardar, onCerrar }: ModalPacienteProps) {
  ...
  const handleChange = <K extends keyof FormPaciente>(
    campo: K,
    valor: FormPaciente[K],
  ) => {
...
export default function GestionPacientes() {
  ...
  const stats = {
    ...
    nuevosMes: pacientes.filter((p) => {
      const partes = p.registro.split("/").map(Number);
  ...
  const guardarPaciente = async (
    ...
    const correoOcupado = pacientes.some(
      (p) =>
        p.correo.toLowerCase() === form.correo.toLowerCase() &&
        p.id !== form.id,
    );
    if (correoOcupado) {
    ...
    if (form.id) {
      await clinicaStore.actualizarPaciente({
        ...(form as Paciente),
        id: form.id,
      });
    } else {
      await clinicaStore.registrarPaciente({
        nombre: form.nombre,
            ...
            <>
              ...
              <div className="border rounded overflow-hidden">
                <div
                  ...
                  style={{
                    letterSpacing: ".03em",
                    gridTemplateColumns: COLUMNAS_TABLA_PACIENTES,
                  }}
                >
                  <span />
                  <span>Paciente</span>
                ...
                {paginados.map((p, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span
                        className={`badge-soft ${activo ? "badge-soft-green" : "badge-soft-gray"}`}
                      >
                        {p.estado}
                      </span>
                      <div className="d-flex align-items-center justify-content-center gap-1">
                        <IconBtn
                          label="Editar"
                          onClick={() => setModalPaciente(p)}
                        >
                          ✎
                        </IconBtn>
                        <IconBtn
                          label={
                            activo ? "Desactivar paciente" : "Activar paciente"
                          }
                          onClick={() =>
                            clinicaStore.toggleEstadoPaciente(p.id)
                          }
                          warn={activo}
                        >
                          {activo ? "⏻" : "✔"}
