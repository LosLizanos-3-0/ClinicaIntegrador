      ...
      <div className="bg-white rounded-4 shadow w-100" style={{ maxWidth: 520 }}>
        ...
        <div className="px-4 py-3 border-top d-flex justify-content-end gap-2">
          ...
          <button onClick={handleSubmit} className="btn btn-primary btn-sm" disabled={guardando}>
            {guardando ? "Guardando…" : esNuevo ? "Registrar paciente" : "Guardar cambios"}
          </button>
        </div>
      </div>
    </div>
...
export default function GestionPacientes() {
  ...
  const guardarPaciente = async (form: FormPaciente): Promise<string | void> => {
    ...
    if (form.id) {
      await clinicaStore.actualizarPaciente({ ...(form as Paciente), id: form.id });
    } else {
  ...
  return (
    <>
      ...
      <div className="bg-white rounded-4 border overflow-hidden">
        ...
        <div className="p-4">
          {cargando ? (
            <p className="fs-6 text-secondary text-center py-5 mb-0">Cargando pacientes…</p>
          ) : (
            <>
              {/* Stats */}
